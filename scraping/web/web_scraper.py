...
def analizar_url_contacto(url: str, origen: str = "web", timeout: int = 6) -> dict | None:
    try:
...
# 1️⃣ Búsqueda para nombre o username concreto (retorna primer contacto útil)
def buscar_por_nombre(username: str, nombre_completo: str | None = None) -> dict:
    ...
    for motor in motores:
        urls = motor(query)
        for url in urls:
            datos = analizar_url_contacto(url, origen="web")
            if datos:
                return datos
...
# 2️⃣ Búsqueda genérica por palabra clave (retorna lista de contactos)
